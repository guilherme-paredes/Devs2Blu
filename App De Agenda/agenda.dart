...
class Agenda {
  ...
  void adicionar() {
    ...
    if (opcao == '1') {
      ...
      String razaoSocial = (stdin.readLineSync() ?? '').trim();
      contato = ContatoEmpresarial(nome, telefone, email, razaoSocial);
    } else {
    ...
    historico.add('Contato adicionado: ${contato.imprimir()}');
  ...
  void editar() {
    ...
    print('\nSelecionado: ${contatos[i].imprimir()}');
    ...
    switch (opcao) {
      case '1':
        contatos[i].nome = lerNome();
        print('\nNome atualizado com sucesso!');
        break;
      case '2':
        contatos[i].telefone = lerTelefone();
        print('\nTelefone atualizado com sucesso!');
        break;
      case '3':
        contatos[i].email = lerEmail();
        print('\nEmail atualizado com sucesso!');
        break;
      case '4':
        contatos[i].editarEspecial();
        break;
      default:
    ...
    historico.add('Contato atualizado: ${contatos[i].imprimir()}');
  ...
  void remover() {
    ...
    stdout.write('\nTem certeza que deseja remover o contato ${contatos[i].nome}? (s/n): ');
    ...
    final removido = contatos.removeAt(i);
    historico.add('Contato removido: ${removido.imprimir()}');
  ...
  void buscar() {
    ...
    for (int i = 0; i < contatos.length; i++) {
      if (contatos[i].nome.toLowerCase().contains(nomeBusca)) {
        print('\nContato encontrado:');
        print(contatos[i].imprimir());
        encontrado = true;
        break;
      }
    }
  ...
  void carregarAgenda() {
    ...
    for (final linha in linhas) {
      ...
      if (campos[0] == 'P' && campos.length == 5) {
        contatos.add(ContatoPessoal.fromCsv(campos));
      } else if (campos[0] == 'E' && campos.length == 5) {
        contatos.add(ContatoEmpresarial.fromCsv(campos));
      }
    }
